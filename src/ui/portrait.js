...
export function portraitSvg(character, opts = {}) {
  const plan = portraitAssetPlan(character, opts);
  const authored = renderAssetPortrait(plan);
  if (authored) return authored;
  const resolved = resolveAppearance(character, opts);
  character = { ...character, appearance: resolved.appearance, maturityRate: resolved.maturityRate };
  const a = character.appearance || {};
  ...
  const hasHair = !HAIRLESS_RACES.has(race)
  ...
  return `<svg viewBox="0 0 ${W} ${H}" width="100%" height="100%" role="img" aria-label="Character portrait" data-asset-variant="${opts.variant === 'sprite' ? 'sprite' : 'sheet'}" xmlns="http://www.w3.org/2000/svg">${parts.join('')}</svg>`;
}
